...
> 采集计划见 [`docs/MEDIA_CAPTURE_PLAN.md`](docs/MEDIA_CAPTURE_PLAN.md)。M1-M4 按采集计划中的文件名与格式补齐；M3 曲线和 M4 GIF/减速截图后续可用真实交互采集替换。

#### M4 — 键盘遥操作端到端（主演示）

![M4 键盘控制机械臂实时运动](media/m4/teleop_keyboard.gif)

验证点：键盘/安全层输出 `/safe_master_pose`，MoveIt Servo 进入 pose-tracking 模式并发布 `/joint_target`，后接 M3 阻抗控制器、CANopen 硬件接口与 MuJoCo 反馈；当前验证日志显示端到端平均延迟约 `1 ms`，低于 `50 ms` 验收目标。

| 端到端延迟测量 | 奇异点/限位减速 |
|---|---|
| ![M4 端到端延迟测量](media/m4/e2e_latency.png) | ![M4 奇异点/限位减速截图](media/m4/singularity_slowdown.png) |
...
验证点：`cartesian_impedance_controller` 作为 ros2_control 插件被 `controller_manager` 加载并激活；M3 按采集计划保留末端跟踪误差、接触柔顺力曲线和控制器 active 状态三类证据。

| 末端跟踪误差 | 接触柔顺力曲线 | 控制器 active 状态 |
|---|---|---|
| ![M3 末端跟踪误差实时曲线](media/m3/ee_tracking_error.png) | ![M3 接触柔顺力曲线](media/m3/contact_compliance_ft.png) | ![M3 控制器 active 状态](media/m3/controller_active.png) |
...
`camera_bridge` 发布 RGB/Depth/CameraInfo @30Hz；`lerobot_recorder` 同步 `joint_states`、`ee_pose`、`ft_sensor`、RGB、Depth，并把夹爪、动作、安全状态、驱动器故障元数据写入 `episode_xxxxxx/train`。训练侧字段契约见 `config/lerobot/act_m6_dataset.yaml`。

```bash
bash scripts/validate_m6_perception_recorder.sh --launch
python3 -c "from datasets import load_from_disk; ds=load_from_disk('.m6_validation/episodes/episode_000000/train'); print(len(ds), ds.features)"
```

*(待补充 `media/m6/lerobot_dataset_features.png` — 多模态 Episode 字段结构截图)*
