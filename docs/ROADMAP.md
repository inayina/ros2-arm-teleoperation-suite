...
│             │              │             │            │          │             │
▼             ▼              ▼             ▼            ▼          ▼             ▼
ros2_control  CANopen DS402  阻抗控制器     MoveIt       安全层     视觉+          遥操作设备
+ MuJoCo      现场总线        (插件)         Servo        + E-Stop   Recorder      可插拔 +
                                                                                合成数据
```

自底向上搭工业栈：先有实时控制框架与物理引擎，再插入现场总线与驱动器，然后是控制律、运动层、安全层，接着补齐感知与数据闭环，最后扩展遥操作设备抽象与合成数据生成能力，对接具身智能训练流水线。
...
| **M4** | `feat/v2-motion-layer` | MoveIt Servo 运动层 | 键盘→`/teleop/cmd_pose`→`safety_monitor`→`/safe_master_pose`→servo→`/joint_target`→阻抗→CAN→MuJoCo 端到端平滑；奇异/限位自动减速；端到端延迟 <50ms |
...
| **M7** | `feat/v2-teleop-synth-data` | 遥操作设备抽象 + 合成数据生成 | `TeleopDriverBase` 接口可插拔；SpaceMouseDriver 接入验证；Domain Randomization 参数化配置；批量 Episode 生成脚本可跑；数据集多样性指标提升 ≥30% |
...
flowchart LR
    ...
    M6 --> M7["M7 遥操作设备可插拔<br/>+ 合成数据 Pipeline"]
```
...
└── feat/v2-teleop-synth-data     ← M7（teleop_input 设备抽象 + synth_data_gen）
```
...
| `teleop_input` | Python | M4（重构：加 `TeleopDriverBase` 抽象） |
| `safety_monitor` | C++ | M5 |
| `camera_bridge` | Python | M6 |
| `lerobot_recorder` | Python | M6 |
| `synth_data_gen` | Python | M7（Domain Randomization + 批量数据生成） |
...
### M3 — 阻抗控制器（插件）
- [x] `cartesian_impedance_controller` 经 pluginlib 导出，`controller_manager` 加载 active
- [x] command interface=`effort`，state interface=`position/velocity`
- [x] 给定 `/joint_target`，末端跟踪（M4 联调验证）
- [x] 接触力 >5N 自动降刚度（柔顺）
- [x] `update()` 稳定 1kHz，可与 `joint_trajectory_controller` 热切

### M4 — MoveIt Servo 运动层
- [x] `teleop_moveit_config` 的 `servo.yaml`/`kinematics.yaml`/SRDF 就位，`servo_node` 直接接收 URDF/SRDF 参数
- [x] `servo_node` 订阅 `/safe_master_pose`（pose 模式）→ 输出 `/joint_target` @125Hz
- [x] `teleop_input` 键盘映射 → `/teleop/cmd_pose`，由 `safety_monitor` 过滤后发布 `/safe_master_pose`
